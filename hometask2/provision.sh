...
&& aws ec2 authorize-security-group-ingress --group-id $sg --protocol tcp --port 24 --cidr 0.0.0.0/0 \
&& aws ec2 create-key-pair --key-name cli-keyPair5 --query 'KeyMaterial5' --output text > cli-keyPair5.pem \
&& chmod 400 cli-keyPair5.pem \
&& aws ec2 run-instances --image-id ami-0533f2ba8a1995cf9 --instance-type t2.micro --count 1 --subnet-id $net1_id --security-group-ids $sg --associate-public-ip-address --key-name cli-keyPair5 \
...
&& echo ssh -i "cli-keyPair5.pem" ec2-user@$user